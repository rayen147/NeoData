...
// Start the session
$subscribe = new subscribes($session_var);


if(isset($_POST['disscount'])){
		if(is_numeric($_POST['disscount'])){
      $subscribe->Insert_or_update_passenger_subs($connection);
	  $offre->ajouter_ou_modifier_offre($connection);
     
    }}else{$subscribe->Insert_or_update_passenger_subs($connection);
      $offre->ajouter_ou_modifier_offre($connection);
	}
if(isset($_GET['DEL'])&&isset($_GET['ID'])){
	$subscribe->Delte_sub($connection);
}
if((!isset($_GET['DEL']))&&isset($_GET['ID'])){
	unset($row);
	$row=Array($subscribe->get_single_subscription($connection));
}else{
	$row=$subscribe->get_all_subscriptions($connection); 
}
?>
...
<div class="container">
  <div class="jumbotron">
      <center>
        ...
        <p><a href="form.php">Cliquez ici </a>réserver un autre abonnent</p>  
        <p>Vous trouverez ci-dessous une liste de toutes les réservations effectuées jusqu'à présent</p>
      </center>
  </div>
</div>
    <script>
...
<div class="container">
 ...
 <?php foreach($row as $s_row) { ?>
      <tr id=<?php echo $s_row['ID']; ?> >
          ...
          <td><?php echo $s_row['Source']; ?></td>
          <td><?php echo $s_row['Destination']; ?></td>
          <td><?php echo $s_row['N_Jours']; ?></td>
          <td><?php echo $s_row['Price']." TND "; ?></td>
          <td></td>
		  
		  <form action="bookpage.php" method="post">
		  <input id="TID" name="TID" type="hidden" value="<?php echo $s_row['ID']; ?>">
		  <input id="CIN" name="CIN" type="hidden" value="<?php echo $s_row['CIN']; ?>">
		  <input id="source" name="source" type="hidden" value="<?php echo $s_row['Source']; ?>">
		  
		  <input id="Destination" name="dest" type="hidden" value="<?php echo $s_row['Destination']; ?>">
		  <input id="prodId" name="prodId" type="hidden" value="<?php echo floor($s_row['Price']/$s_row['N_Jours']); ?>">
