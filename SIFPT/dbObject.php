 ...
 * @link      http://github.com/joshcam/PHP-PD0-Database-Class
 * @version   2.9-master
 *
 * @method int count ()
...
class dbObject {
    /**
     * Working instance of PD0 created earlier
     *
     * @var PD0
     */
    private $db;
    ...
    private function byId ($id, $fields = null) {
        $this->db->where (PD0::$prefix . $this->dbTable . '.' . $this->primaryKey, $id);
    ...
    private function join ($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null) {
        ...
        if (!$primaryKey)
            $primaryKey = PD0::$prefix . $joinObj->dbTable . "." . $joinObj->primaryKey;
		
        if (!strchr ($key, '.'))
            $joinStr = PD0::$prefix . $this->dbTable . ".{$key} = " . $primaryKey;
        else
            $joinStr = PD0::$prefix . "{$key} = " . $primaryKey;
     ...
     *
     * Provides magic access to private functions of the class and native public PDO functions
     *
     * @param string $method
     * @param mixed $arg
     *
